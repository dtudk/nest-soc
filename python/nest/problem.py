...
def advance_step_area(boundary:BoundaryData,
                      cell:Cell,
                      **kwargs)->BoundaryData:
    """
    ...
    * auto-diff package is not compatible, because it has not been updated to new numpy versions
    ...
    def find_voltage(x):
        return boundary.V - cell.V(x,boundary.T,Ps_fuel(x),Ps_air(x),**kwargs)
    
    j = newton(find_voltage, boundary.j) # use previous step current density or user-input guess
...
def solve_area(boundary:BoundaryData,
               cell:Cell,
               **kwargs)->np.ndarray:
    """
    ...
    for e in range(cell.elements):
        ...
        boundary = advance_step_area(boundary,cell,**kwargs_el)
