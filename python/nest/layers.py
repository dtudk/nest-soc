...
from nest.idealGas import Specie,Mixture,BasicSpecies
...
class Kinetic:
    ...
    Eact : activation energy for Arrhenius equation
    ...
    * The half-reaction is implicitly approximated as an heteregenous reaction happening in the interface of electrode/electrolyte by taking the area specific exchange current density formulation [A/m2]
      a discussion on the differences between heterogenous and homogeneous treatment is given in Ref. [1]
    ...
    * Nernst equation for half-reaction voltages.
    ...
    """
    def __init__(self,mix=Mixture((BasicSpecies.H2,BasicSpecies.O2)),coeff=np.array([0,0]),n_e=2,alpha=0.5,beta=0.5,gamma=0,p=np.array([0,0]),theta=1,Eact=0):
    ...
    def j_0(self,T:float,Ps:np.ndarray)->float:
        ...
        P_0 = 101325    # Reference pressure [Pa] - Source: https://doi.org/10.1016/j.pecs.2020.100902
        return self.gamma*T**self.theta*np.exp(-self.Eact/(R*T))*np.prod((Ps/P_0)**self.p)
    def molFlux(self,j:float)->float:
        """ Molar flux [mol/m^2]
...
class ButlerVolmer(Kinetic):
    ...
    p (numpy.ndarray) : exponential factors for partial pressure dependency
    theta : exponential factor for Arrhenius equation
    Eact : activation energy for Arrhenius equation [J/mol.K]
    
    Returns:
    Kinetic : representation of electrochemical half-reaction

    Comments:
    Buttler-Volmer is not a theoretically rigourous model in most cases, a brief explanation is given in Ref. [1]
    ...
    def djdV(self,V:float,T:float,j0:float)->float:
        ...
        F = 96485.33212    # s.A/mol - Source: https://physics.nist.gov/cuu/Constants/
        R = 8.314462618    # J/mol.K - Source: https://physics.nist.gov/cuu/Constants/
        return j0*(self.alpha*self.n_e*F/(R*T)*np.exp(self.alpha*self.n_e*F*V/(R*T))+self.beta*self.n_e*F/(R*T)*np.exp(-self.beta*self.n_e*F*V/(R*T)))
    def djdV2(self,V:float,T:float,j0:float)->float:
        ...
        F = 96485.33212    # s.A/mol - Source: https://physics.nist.gov/cuu/Constants/
        R = 8.314462618    # J/mol.K - Source: https://physics.nist.gov/cuu/Constants/
        return j0*((self.alpha*self.n_e*F/(R*T))**2*np.exp(self.alpha*self.n_e*F*V/(R*T))-(self.beta*self.n_e*F/(R*T))**2*np.exp(-self.beta*self.n_e*F*V/(R*T)))
    def V(self,j:float,T:float,j0:float)->float:
        ...
        Comments:
        * Linear, Sinh and Tafel approximations come from the mathematical approximations of BV under restriced conditions. 
        * This function is not compatible with JAX package, because of the use of scipy.optimize.newton function
        """
        F = 96485.33212    # s.A/mol - Source: https://physics.nist.gov/cuu/Constants/
        R = 8.314462618    # J/mol.K - Source: https://physics.nist.gov/cuu/Constants/
        # Finding V guess
        ...
        # Root finding
        return newton(lambda x:self.j(x,T,j0)-j,guess,fprime=lambda x:self.djdV(x,T,j0),fprime2=lambda x:self.djdV2(x,T,j0))
...
class Conductivity:
    ...
    Eact (float) : Activation energy [J/mol.K]
    theta (float) : Temperature exponential coefficient

    Returns:
    (float) : Conductivity model
    """
    def __init__(self,sigma0=np.inf,Eact=0.0,theta=-1):
        self.sigma0 = sigma0
        self.Eact = Eact
        self.theta = theta
    def sigma(self,T:float)->float:
        ...
        """
        R = 8.314462618    # J/mol.K - Source: https://physics.nist.gov/cuu/Constants/
        return self.sigma0*T**self.theta*np.exp(-self.Eact/(R*T))
...
class BinaryFick(PorousTransport):
    def D_eff(self,T:float,P:float,mix:Mixture)->np.ndarray:
        ...
        Comments:
        Bosanquet formula assumes: (i) binary system, (ii) equimolar counter transport, (iii) constant pressure [1]
        ...
        if type(mix.species)==Specie:
            ...
            return np.array([ 1/((self.epsilon/self.tau*self.D_knudsen(T,mix.species[i]))**(-1)+(self.epsilon/self.tau*D_ij)**(-1)) for i in range(2)] )
        else:
            raise ValueError("Binary Fick is only valid for pure substance or binary mixture")
    def dPdl(self,molFlux:float,T:float,P:float,mix:Mixture)->np.ndarray:
        """ Fick's first law of diffusion - Pressure
        
        Parameters:
        molFlux (float) : molar flux [mol/m^2]
        T (float) : Temperature [K]
        P (float) : Pressure [Pa]
        ...
        * Function also assumes -molFlux as boundary condition for molar flux at x = x_max
        """
        R = 8.314462618    # J/mol.K - Source: https://physics.nist.gov/cuu/Constants/
        return molFlux/self.D_eff(T,P,mix)*R*T
    # To-do: dP/dt which takes P_t, and two boundary conditions (P_bulk, molFlux) to calculate P_t+dt : mass-transport transiency
...
class Layer:
    ...
    * delta reflects the initial thickness of the layer, as this may change because of degradation.
    """
    def __init__(self,delta=0.0,kinetic=Kinetic(),conductivity=Conductivity(),transport=PorousTransport()):
        self.delta = delta
        self.kinetic = kinetic
        self.conductivity = conductivity
        self.transport=transport
    def V_ohm(self,j:float,T:float)->float:
        """ Ohmic voltage [V]
    ...
    def Ps_star(self,j,T,Ps):
        ...
        """
        return Ps + self.transport.dPdl(self.kinetic.molFlux(j),T,sum(Ps),self.kinetic.mix)*self.delta
