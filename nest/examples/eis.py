...
def eis():
    ...
    for i, elements in enumerate([10]):
        ...
        YSZ_CGO = layers.Layer(
            delta=4e-6,
            conductivity=layers.Conductivity(
                sigma0=1715, E_act=8785 * 8.314510, theta=0
            ),
        )
        CGO = layers.Layer(
            delta=10e-6 - 2e-6,
        ...
        V_nernst = cellObject.V_nernst(
            T, P * n_fuel / n_fuel_total, P * n_air / n_air_total
        )
        boundaryObject = cell.BoundaryData(
            V=V_nernst, n_fuel=n_fuel, n_air=n_air, T=T, P=P
        )
        ...
        j_avg = np.mean(
            sol.y[:, 2 * cellObject.elements : 3 * cellObject.elements], axis=1
        )
        offset = 1e-3
