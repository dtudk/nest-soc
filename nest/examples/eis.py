...
def resfn(t, y, yp, res, exampleCell, conditions):
    ...
    for i in range(n):
        ...
        for s in range(n_species_fuel):
            ...
            J_surf = -1*(y[index_c]-cf_x) # OBS: high flux to ensure equilibrium at the interface
        ...
        for z in range(n_z-1): # Forward difference finite elements
            ...
            for s in range(n_species_fuel):
                ...
                c_bottom = y[index_c+1]

                # Diffusion to get Js - simplified
                res[index_J] = (c_bottom-cs[s])/dz_f + rhs[s]
        ...
        for z in range(n_z):
            for s in range(n_species_fuel):
                ...
                if z == 0:
                    ...
                    J_top = -1*(c_y-c_top) # High flux to ensure equilibrium at the interface
                    J_y = y[index_J]
                elif z < n_z-1:
                    J_top = y[index_J-1]
                    J_y = y[index_J]
