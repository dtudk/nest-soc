    ...
    * Impact on electronic conduction resistance is not modeled
    ...
    def material_dt(self,
        ...
        * The model here is based on Ref. [1]
        ...
        xi_ed = self.psi_ed/(r/1e6)**3/(self.psi_el/self.r0_el**3+self.psi_ed/(r/1e6)**3)
        xi_el = self.psi_el/self.r0_el**3/(self.psi_el/self.r0_el**3+self.psi_ed/(r/1e6)**3)
        ...
        n_tot = (1-self.epsilon)/(4/3*pi*self.r0_el**3*xi_el+4/3*pi*(r/1e6)**3*xi_ed)
        s_el = 2*pi*self.r0_el**2*(2-(1-np.cos(self.theta))*(Z_el_el+Z_el_ed))
        A_el = s_el*n_tot*xi_el/1E6 # 1/m * m/1E6 microM
        # Rate equation
        k = self.alpha/8*self.psi_ed/(1-self.psi_ed)/A_el*(Ps_fuel_star[1]/1e5)/(Ps_fuel_star[0]/1e5)**0.5
